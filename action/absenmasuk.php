...
if (mysqli_num_rows($sql) >= 1) {
 ...
 $update = "UPDATE absen SET tgl_absen='$tgl', jam_datang='$jam', ket='Hadir' WHERE id_guru='$id' AND tgl_absen = '$tgl'";

 $sql2 = mysqli_query($conn, $update);
 if ($sql2) {

  echo '<script language="javascript">
       window.alert("Anda berhasil melakukan Absen masuk!");
       window.location.href="../user/index.php?page=absen";
     </script>';
 } else {
  echo '<script language="javascript">
  window.alert("Anda gagal melakukan Absen masuk!");
  window.location.href="../user/index.php?page=absen";
</script>';
 }
} else {
